...
function showCard(){
    ...
    document.getElementById("card").style.display = "bloclk";
    var card = document.getElementById("card");
    card.innerHTML = `<div class="card-body">
                <h6 class="card-header text-secondary mb-3" id="name">Name: ${parseData} </h6>
                <p class="card-text text-secondary" id="roll">Roll Number: ${parseData} </p>
                <p class="card-text text-secondary " id="roll">Email: ${parseData}</p>
                <button onclick="redirect()" class="btn btn-danger">Log out</button>
            </div>`;
